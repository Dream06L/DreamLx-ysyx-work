...
//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = (word_t)(int8_t)Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));//读出内存最低8位，然后0扩展
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4));
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1+imm);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc;s->dnpc=(src1+imm)&0xfffffffe);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (word_t)((int32_t)src1 >> BITS(imm, 4, 0)));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = (word_t)(int16_t)Mr( src1 + imm,2));

  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));

  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc;s->dnpc=(s->pc+imm)&0xfffffffe);
...
int isa_exec_once(Decode *s) {
  s->isa.inst = inst_fetch(&s->snpc, 4);//取指，snpc+4
